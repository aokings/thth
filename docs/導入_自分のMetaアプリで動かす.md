...
## 読む順番 —— **最短の道**（2026-09-15 に並べ替え）

**節番号は付け替えていません**（`thth doctor` の「次の一手」が §3・§4・§5 と名指しで指すため）。
番号の順ではなく、**この順に読んでください**。

| 順 | 節 | 何をする |
|---|---|---|
| 1 | §1 | VM・Python 3.12・git |
| 2 | §2 | Meta アプリ・tester 招待・**Threads アプリ ID** |
| 3 | **§4** | アカウント台帳 `accounts/<account>.json` |
| 4 | **§5-(a)** | **管理画面の「ユーザートークン生成ツール」→ `thth token set`** |
| 5 | §6 | timer |
| 6 | §7 | 確かめ方（doctor → lint → board → dry-run） |
| （7） | §8 | 配布の枝 |

**§3（`app.env`）と §5-(b)（`thth auth`）は迂回路です。** OAuth の往復をこちらでやる道で、
**権限の内訳を変えたいときだけ**通ります（生成ツールは、そのアカウントが過去に承認した
範囲でしかトークンを出しません・§2「権限を増やしたら」）。**最短の道では両方とも飛ばせます。**
...
**`https://thth.me/callback/` は、誰の Meta アプリの受け口としても使えます**——戻ってきた
`code` と `state` を組み直した**戻り URL 全体を画面に表示するだけ**で、Worker はどこへも
送らず保存もしません（`callback/src/index.js`・**L1**。Cloudflare 側のアクセスログは別）。
**自分で HTTPS の受け口を立てなくて構いません**（`redirect_uri` にこの URL を書き、
自分の Meta アプリのコールバック 3 欄にも同じ値を登録するだけ）。
...
## 3.（迂回路）`~/.config/thth/app.env` —— **§5-(b) の `thth auth` を使うときだけ**

### **ふつうはこの節を飛ばします**

**最短の道（§5-(a) の生成ツール → `thth token set`）なら `app.env` は要りません。この節を飛ばして §4 へ進んでください**（設計 §4.2「置かないものは漏れない」）。読むのは**権限の内訳を変えたくなった日**だけです。
...
## 5. トークン（`thth token set`・`thth auth`・60 日更新）

**トークンはアカウントごとに 1 本、`~/.config/thth/<account>.token` に 600 で置きます。repo には入れません**（設計 §3.1・**L1**）。

**道は 2 つ。** **(a) が最短の道**で、ふつうはこれだけで終わります。(b) は**権限の内訳を変えたい日**の迂回路です。

### 5-(a). 生成ツール → `thth token set` —— **最短の道。ここだけ読めば動きます**

管理画面の「ユースケース → Threads API → 設定」の下に「ユーザートークン生成ツール」があり、tester ごとに長期トークンをボタン 1 つで発行できます（設計 §4.2・**L3**）。**OAuth の往復も app secret も要りません**——だから §3（`app.env`）も、§2-8 のコールバック URL 登録も要りません。
...
### 5-(b).（迂回路）`thth auth` —— **権限の内訳を変えたいときだけ**

OAuth の往復をこちらでやる道です。**要るのはこの 2 つの日だけ**: ① 権限の内訳を変える（生成ツールは過去に承認した範囲しか出しません・§2「権限を増やしたら」）、② tester 以外を扱う。**先に §3（`app.env`）と §2-8（コールバック URL 3 欄）が要ります。**

**受け口は借りられます**——`redirect_uri` には `https://thth.me/callback/` をそのまま使えます（戻り URL 全体を表示するだけで何も送りません・§2「コールバック URL は 3 つセット」）。自分で HTTPS の受け口を立てる必要はありません。
