...
2026-09-18。`serve-reports` は専用Unix環境のレポートを、機械クライアントへ返すための限定的な入口です。2.5.0 から入っていますが**開発版**で、stdlib `HTTPServer` による直列処理で、インターネット向け本番サーバ、本人認証、投稿承認、OSサンドボックスの完成を意味しません。
