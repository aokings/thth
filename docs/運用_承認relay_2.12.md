...
**ただし 1 回の表示に 20 枚ぶんの枠を使うので、20 枚の承認ページを 1 分に 7 回以上読み直すと 429 になります**（120 ÷ 20 ＝ 6 回まで）。`<img>` は `loading="lazy"` なので画面外の画像は読まれず、実際に当たるのは「全部を画面に出したまま何度も読み直した」ときです。429 に当たったら 1 分待って読み直してください——承認の 600 秒の期限はそのままです。preview 専用の枠（purpose=preview の別鍵）は作っていません。`/m/…` の制限は DO を呼ぶ前に効かせており、purpose は DO の中にしか無いからです（先に DO を呼ぶと、制限の目的そのものが崩れます）。

承認ページは GET のたびに画像 preview が**いま生きているか**を media object に尋ねます。1 枚でも生きていなければ、承認フォームを出さずに「添付を表示できないため、この承認ページは使えません。サーバから新しく承認を求めてください。」だけを出します（状態は 200）。図の説明にも「画像が表示されない場合は承認しないでください」と添えます——**見えなかった添付は承認されていない**。
...
承認が決着（承認・受領・期限切れ）すると、その session が出していた preview は即座に失効します（`/m/…` は 410）。発行のときに preview の寿命は session の期限まで引き下げてあるので、失効の RPC が届かなくても session より長くは生きません。
