...
- **上の提案は、書いた直後に事実で答えが出た（2026-09-14 15:5x・運用が VM で実測・L1）。** v2.0.2 の
  `thth doctor masaru-threads` は 6 つの新しい権限のうち読み取りの 4 つ（`keyword_search` 1 件・`mentions` 0 件・
  `profile_lookup` {"username":"threads"}・`location_search` 30 件）を**実際に叩いて全部 ○**、`delete`・
  `share_to_instagram` も `/debug_token` の一覧に**乗っている**。**tester のトークンで、App Review なしに 11 権限が
  使えている。** したがって**自社 4 アカウントの運用には App Review は要らない**（他の 3 本は再認可すれば同じになる）。
  App Review が要るのは、**役割を持たない他人のアカウントにこのアプリを使わせるとき**だけで、THTH の分散の形
  （利用者は自分の Meta アプリを持ち込む）ではその場面が無い。**以下の台本は、将来その場面が来たときのために残す。**
