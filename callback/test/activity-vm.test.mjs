...
before(async()=>{
  ...
  const files=['src/worker.js','src/invite.js','src/invite-object.js','src/activity.js','src/api.js','src/login.js','src/login-object.js','src/media.js','src/media-object.js','src/index.js','src/relay.js','src/relay-object.js','src/person.js','src/person-object.js','src/deletion.js','src/deletion-object.js'];
...
function vm(...args){
  ...
  const result=spawnSync(pythonForTests(),['test/activity-vm.py',...args],{cwd,encoding:'utf8',timeout:60000,env:{PATH:process.env.PATH,HOME:join(directory,'home'),
    ...
    THTH_TEST_ALLOW_HTTP:'1',THTH_RELAY_BASE_URL:origin}});
