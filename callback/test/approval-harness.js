...
import {ApprovalPerson,ApprovalSession} from '../src/approval-object.js';
import {personStub,sessionStub} from '../src/approval.js';
export {AuthRelay} from '../src/relay-object.js';
export class TestPerson extends ApprovalPerson {
  now(){return this.clock??Date.now();}
  configure(body){this.clock=body.clock;this.fault=body.fault;this.revokeAfterCheck=body.revokeAfterCheck;}
...
export default {async fetch(request,env){
  const match=/^\/__approval\/(person|session)\/(.+)$/.exec(new URL(request.url).pathname);
  if(match){const stub=await(match[1]==='person'?personStub(env,match[2]):sessionStub(env,match[2]));
