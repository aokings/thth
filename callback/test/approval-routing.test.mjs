...
test('real approval routing bypasses assets, two DOs survive restart and consumed receipt stays consumed',{timeout:40000},async()=>{
  ...
  const signed=async(type,id,operation,body)=>{
    ...
    const signature=crypto(['dgst','-sha256','-sign',key,'-sigopt','rsa_padding_mode:pss','-sigopt','rsa_pss_saltlen:32'],Buffer.from(canonical('POST',path,type==='person'?'operator':'job',id,operation,time,nonce,hash(raw)))).toString('base64url');hidden.push(signature);
  ...
  }finally{await stop();await rm(dir,{recursive:true,force:true});const hits=logs.filter(line=>hidden.some(secret=>line.includes(secret))).length;assert.equal(hits,0,'HTTP runtime logs contain private data');console.log('approval routing log scan: '+logs.length+' chunks, '+hits+' hits; 4 local starts');}
});
