...
// Deliberate, known unavailability. Only this sentinel becomes a 503; anything
// else is an unexpected fault and answers 500 with no detail from its message.
export class MediaUnavailable extends Error{}
async function allowed(binding,key){if(!binding)throw new MediaUnavailable();return (await binding.limit({key:await digest(key)})).success===true;}
...
export async function mediaRequest(request,env,url){
  ...
  }catch(error){return error instanceof MediaUnavailable?reply(503,{error:'media_unavailable'}):reply(500,{error:'media_internal'});}
}
