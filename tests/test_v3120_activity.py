...
見るのは:
  ...
  - `thth admin invite create --approval`。
"""
...
def test_account_setは台帳を書き換え変更ログに残す_by必須(env):
    assert thth("account", "set", "alpha", "approval", "publish").returncode == 2
    done = thth("account", "set", "alpha", "approval", "none", "--by", "masaru")
    assert done.returncode == 0, done.stderr
    assert read_ledger(env)["approval"] == "none"
    ...
    assert [row["diff"] for row in rows] == [{"approval": ["all", "none"]}, {"burst_count": [3, 2]},
                                             {"burst_minutes": [10, 20]}, {"daily_max_posts": [8, 30]},
                                             {"min_interval_hours": [0, 1.5]}]
    assert all(row["by"] == "masaru" and row["via"] == "cli" for row in rows)


@pytest.mark.parametrize("args", [("stopped", "0"), ("approval", "sometimes"), ("daily_max_posts", "-1"),
...
def test_account_statusとresume(env):
    ...
    assert row["settings"]["approval"] == "all" and row["stopped"] is None and row["today"]["posts"] == 0
...
def test_mcpのthth_settingsは締める向きだけ(env, monkeypatch):
    ...
    assert read["settings"]["approval"] == "all" and "burst_count" in read["keys"]
    ledger(env, approval="none")
    for key, value in (("approval", "publish"), ("daily_max_posts", "4"), ("burst_count", "2"),
    ...
    cfg = read_ledger(env)
    assert cfg["approval"] == "publish" and cfg["daily_max_posts"] == 4 and cfg["burst"] == {"count": 2, "minutes": 30}
    assert cfg["hold_minutes"] == 5 and cfg["min_interval_hours"] == 8
    for key, value in (("approval", "none"), ("daily_max_posts", "5"), ("burst_count", "3"),
    ...
    # 変えられるのは approval と数値だけ。
    for key in ("stopped", "production", "scheduled", "quiet_hours"):
...
def test_返信には最短間隔を掛けない_上限には数える(env, publisher, monkeypatch):
    ...
    ledger(env, approval="none", min_interval_hours=6, daily_max_posts=2)
...
def test_scheduledでない口座の予約はschedule_unavailable(env, remote, mode):
    ...
    if mode == "none":
        assert str(refused(env, operation="approval_request", draft_id=one["draft_id"])) == "schedule_unavailable"
    else:
        assert "approval_url" in call(env, operation="approval_request", draft_id=one["draft_id"])
...
def test_scheduledでない口座で猶予つきの今すぐも断る_下書きも作らない(env, publisher):
    ledger(env, approval="none", scheduled=False, hold_minutes=10)
...
def test_要約は本文の先頭60字だけ_秘密を含まない(env, worker, publisher):
    ...
    assert summary["account"] == "alpha" and summary["approval"] == "none"
...
def test_招待のapprovalは台帳に書き一覧に出る(monkeypatch, tmp_path):
    ...
    row = invites.create(media="threads", project="meta-review", production=True, by="masaru", approval="all")
    assert row["approval"] == "all"
    record = invites.STORE.get(row["invite_id"])
    assert record["approval"] == "all"
    assert invites._ledger(record)["approval"] == "all"
    plain = invites.create(media="threads", project="friend", by="masaru")
    assert plain["approval"] == "none" and invites._ledger(invites.STORE.get(plain["invite_id"]))["approval"] == "none"
    with pytest.raises(invites.InviteError) as caught:
        invites.create(media="threads", project="x", by="masaru", approval="sometimes")
    assert str(caught.value) == "invalid_approval"
    rows, _ = invites.list_invites()
    assert {r["invite_id"]: r["approval"] for r in rows} == {row["invite_id"]: "all", plain["invite_id"]: "none"}
    created = admin_log.read(event="invite_created")[0]
    assert created[0]["diff"]["approval"] == [None, "all"]
