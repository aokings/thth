"""3.12.0 段 1〜2: 承認を選ぶ（`approval`）・承認なしの道・安全装置（`thth/guard.py`）。

見るのは:
  - 台帳の `approval` と安全装置の数値の検証（知らない値は断る）と既定。
  - 招待で用意した既存の口座（approval の項目が無い）は all として扱う。
  - approval ごとの道: none は公開・削除・予約がその場、publish は削除だけその場、all は全部 job。
  - 直接の公開・削除・予約の記録に via と資格の id（credential_digest の先頭 12 文字）が残る。
  - 最短間隔・1 日の上限・削除の上限で断り、次に出せる時刻を添える。
  - burst を超える依頼は口座を止めて断り、止まった口座は公開・削除・予約（承認の道も）を断る。
"""
...
def test_approvalの既定はnone_招待の既存の口座はall_明記すればその値():
    assert accounts.approval_mode({}) == 'none'
    invite = {'provenance': {'created_via': 'invite'}}
    assert accounts.approval_mode(invite) == 'all'
    assert accounts.approval_mode({**invite, 'approval': 'none'}) == 'none'
    assert accounts.approval_mode({'provenance': {'created_via': 'cli'}}) == 'none'
...
@pytest.mark.parametrize('key,value', [
    ('approval', 'sometimes'), ('approval', True), ('daily_max_posts', -1), ('daily_max_posts', True),
...
def test_知らない値は台帳の読み込みで断る(env, key, value):
    ...
    assert ('invalid_approval' if key == 'approval' else 'invalid_guard_limits') in str(caught.value)


def test_招待の口座の新しい台帳はnoneを明記する(monkeypatch):
    from thth import invites
    data = invites._ledger({'account': 'inv-x-000001', 'project': 'x', 'production': False,
                            'invite_id': 'a' * 16, 'by': 'masaru'})
    assert data['approval'] == 'none' and data['provenance']['created_via'] == 'invite'
...
def test_noneは今すぐの公開をその場で出し_viaと資格のidを残す(env, remote, publisher):
    ledger(env, approval='none')
...
def test_noneは削除をその場で行い_記録にviaと資格のidを足す(env, remote, publisher):
    ledger(env, approval='none')
...
@pytest.mark.parametrize('operation', ['schedule_request', 'approval_request'])
def test_noneは予約をその場でqueueに刻む_承認と同じ形(env, remote, operation):
    ledger(env, approval='none')
    one = draft(env)
    result = call(env, operation=operation, draft_id=one['draft_id'])
...
def test_publishは公開を承認jobに_削除はその場(env, remote, publisher):
    ledger(env, approval='publish')
    first = call(env, operation='send_request', body='承認を待つ本文')
    assert first['approval_url'].startswith('https://thth.me/approve/') and publisher == []
    one = draft(env)
    scheduled = call(env, operation='schedule_request', draft_id=one['draft_id'])
    assert scheduled['kind'] == 'approve' and 'approval_url' in scheduled
    # 削除はその場（承認ページを通らない）。消す相手の記録を置いておく。
    sent.write(str(env['root'] / 'state/alpha'), post_id='777', text='前の投稿', body_hash='x', sent_at=jst.iso())
    result = call(env, operation='retract_request', post_id='777', reason='訂正')
    assert result['status'] == 'retracted' and publisher == [('delete', '777')]


def test_allは削除も承認job(env, remote, publisher):
    ledger(env, approval='all')
    sent.write(str(env['root'] / 'state/alpha'), post_id='777', text='前の投稿', body_hash='x', sent_at=jst.iso())
    result = call(env, operation='retract_request', post_id='777', reason='訂正')
    assert result['kind'] == 'retract' and 'approval_url' in result and publisher == []


def test_招待の既存の口座はall扱いで承認jobになる(env, remote, publisher):
    ledger(env, approval=None, provenance={'created_at': jst.iso(), 'created_by': 'masaru',
                                            'created_via': 'invite', 'created_host': 'vm'})
    result = call(env, operation='send_request', body='本文')
    assert 'approval_url' in result and publisher == []


def test_hold_minutesがあれば今すぐもその分先の予約になる(env, remote, publisher):
    ledger(env, approval='none', hold_minutes=15)
    before = jst.now_jst()
...
def test_最短間隔で断り_次に出せる時刻を添える(env, publisher):
    ledger(env, approval='none', min_interval_hours=6)
...
def test_1日の公開の上限で断る(env, publisher):
    ledger(env, approval='none', daily_max_posts=1)
    call(env, operation='send_request', body='一本目')
    exc = refused(env, operation='send_request', body='二本目')
...
def test_1日の削除の上限で断る(env, publisher):
    ledger(env, approval='none', daily_max_retracts=0)
    call(env, operation='send_request', body='一本目')
...
def test_burstを超える依頼は口座を止めて断り_止まった口座は全部断る(env, remote, publisher, monkeypatch):
    ledger(env, approval='none', burst={'count': 2, 'minutes': 10})
    ...
    ledger(env, approval='all')
    assert str(refused(env, operation='approval_request', draft_id=one['draft_id'])) == 'account_stopped'
    assert remote.rows == {} and publisher == []
    # 数え方は間隔の外に出ても変わらない: 持ち主が戻すまで止まったまま。
    ledger(env, approval='none', burst={'count': 100, 'minutes': 10})
...
def test_承認済みのjobも止まった口座では出さない(env, remote, publisher):
    ledger(env, approval='all')
    from thth import approval_jobs as jobs
    result = call(env, operation='send_request', body='承認済みの本文')
