"""3.12.0 §6-1・§6-6: 承認の常駐は、ディスクの版が動いたら自分で終わり（systemd が起こし直す）、
...
見るのは:
  ...
  - `thth approval-worker` は ssh で umask 0002 のまま打っても、作るものは 700/600。
"""
...
from thth import approval_jobs, cli, worker_version
...
def worker_env(tmp_path, monkeypatch):
    ...
    calls = []
    monkeypatch.setattr(approval_jobs, "run_once", lambda credentials: calls.append(credentials))
    monkeypatch.setattr(approval_jobs.time, "sleep", lambda seconds: None)
...
def test_ディスクの版が動いたら常駐は終わり_起動した版を残す(worker_env, monkeypatch, capsys):
    ...
    rc = approval_jobs.command(SimpleNamespace(credentials="/private/c.json", once=False))
    assert rc == worker_version.EXIT_MOVED and rc != 0
    assert len(calls) == 3
    err = capsys.readouterr().err
    assert "approval_worker_restart: 3.11.1（aaaaaaa） -> 3.12.0（bbbbbbb）" in err
...
def test_onceは記録も比較もしない_umaskは077(worker_env, monkeypatch):
    ...
    assert approval_jobs.command(SimpleNamespace(credentials="/private/c.json", once=True)) == 0
...
def test_手で打った常駐が作るものは700と600(worker_env, monkeypatch):
    ...
    monkeypatch.setattr(approval_jobs, "run_once", run_once)
    assert approval_jobs.command(SimpleNamespace(credentials="/private/c.json", once=True)) == 0
    (directory,) = made
...
def test_boardの行(monkeypatch):
    ...
    assert line == "承認の常駐: 3.11.1（aaaaaaa）  pid 4242・起動 2026-09-25T06:05:00+09:00"
