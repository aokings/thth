...
def metadata(value, base, required_scopes=None):
    required_scopes=SCOPES if required_scopes is None else required_scopes
    ...
    for key,required in [('code_challenge_methods_supported',['S256']),('grant_types_supported',['authorization_code']),
                         ('response_types_supported',['code']),('scopes_supported',required_scopes),
...
def valid_client(data, base, required_scopes=None):
    required_scopes=SCOPES if required_scopes is None else required_scopes
    if (not isinstance(data,dict) or set(data)!={'client_id','client_secret','instance','redirect_uri','scopes','metadata','created_at'}
            or data['instance']!=base or data['redirect_uri']!=CALLBACK
            or type(data['scopes']) is not list or not all(type(v) is str for v in data['scopes'])
            or len(data['scopes'])!=len(set(data['scopes'])) or set(data['scopes'])!=set(required_scopes)):
    ...
    for key in ('client_id','client_secret'):
        ...
        secret(value)
    metadata(data['metadata'],base,required_scopes)
    return data
...
class MastodonAuthProfile(AuthProfile):
    ...
    def prepare(cls,cfg,*,redirect_uri=None,rehearse=False,resume=False,by=None):
        ...
        path=authclients.path_for('mastodon',base,cfg,required_scopes=SCOPES)
        ...
        if rehearse or resume:
            try:
                ...
                if stored is not None:client=valid_client(stored,base)
            ...
            if stored is None:
                ...
                try:client=valid_client(legacy,base,LEGACY_SCOPES)
                except (OSError,ValueError):raise FlowError('auth_client_invalid') from None
                path=legacy_path;selected_scopes=list(LEGACY_SCOPES)
        else:
        ...
        if resume:
            from .. import authflow
            session=authflow._read_session(cfg['account'])
            if session and session.get('binding')!=profile.binding(cfg):raise FlowError('auth_restart_required')
        return profile

    def validate(self):
        origin(self.instance)
        valid_client(authclients.read(self.client_path),self.instance,self.scopes)

    def current_client(self):
        value=valid_client(authclients.read(self.client_path),self.instance,self.scopes)
    ...
    def authorize(self,session):
        ...
        return self.instance+'/oauth/authorize?'+urllib.parse.urlencode(dict(response_type='code',client_id=self.client_id,
            redirect_uri=CALLBACK,scope=' '.join(self.scopes),state=session['state'],code_challenge=challenge,
    ...
    def exchange(self,code_value,session,account_cfg,*,log):
        ...
        if (not isinstance(token,str) or not token or len(token)>16384 or any(ord(c)<32 or ord(c)==127 for c in token) or not isinstance(scope,str)
                or not set(self.scopes)<=set(scope.split()) or str(body.get('token_type','')).lower()!='bearer'):
        ...
        return dict(access_token=token,user_id=uid,username=acct,obtained_at=jst.iso(),no_expiry=True,
                    scopes=scope.split(),scopes_source='response',client_scope_generation=authclients.scope_generation(self.scopes))


def client_for_token(cfg,token):
    """Local revoke selects the client generation that issued this token."""
    base=origin(cfg.get('instance'));generation=token.get('client_scope_generation')
