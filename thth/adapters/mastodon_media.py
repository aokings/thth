...
def intent_error(manifest):
    ...
    if any(x['role']!='media' or x['kind'] not in ('image','video','audio') or x['format'] not in (set(MIME)|{'wav','flac','mp3','ogg','ogg_vorbis'}) for x in manifest['files']):
...
def mime_for(row,cap=None):
    if row['kind']=='audio':
        choices={'mp4':('audio/mp4','audio/m4a','audio/x-m4a'),'mov':('video/quicktime',),'wav':('audio/wav','audio/wave','audio/x-wav','audio/vnd.wave'),'flac':('audio/flac','audio/x-flac'),'mp3':('audio/mpeg','audio/mp3'),'ogg':('audio/ogg','audio/opus'),'ogg_vorbis':('audio/ogg',)}.get(row['format'],())
