...
def reaction(name, medium, engagement, posted, now):
    ...
    if ledger.get('broken'):
        return None, None
    fetched = any(posted <= at <= now for f in ledger['fetches']
                  if (at := c._timestamp(f.get('collected_at'))) is not None)
    direct = []
...
def summarize(name, cfg, eng, by_id, start, end, now, min_n):
    ...
    for pid, rows in grouped.items():
        ...
        post = by_id.get(pid)
        if posted is None or not start <= posted < end or not isinstance(row.get('root_post'), str) or not row['root_post']:
            continue
        ...
        branches[row['root_post']].append({**curve, 'reply_to': row.get('reply_to'),
    ...
    result = [{'root_post': root, 'replies': sorted(rows, key=lambda x: (x['posted_at'], x['post_id']))}
              for root, rows in sorted(branches.items())]
    strata = {}
