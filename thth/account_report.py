...
def topic_performance(account_name: str, *, limit: int = REMOTE_LIMIT) -> dict:
    ...
    **刺さったか**＝同じトピックの中での散らばり（最小〜最大）。
    ...
    202〜574 views、nigamilab のトピック無しは 1 view（****訂正 2026-09-12**: この「1 view」は**経過が数時間の疎通確認投稿**で、同じ投稿が 9/12 時点で **112 views**。**400 倍の大半は経過時間だった。**トピックが効かないという意味ではなく、**この数字では判定できない。****）。
    **届ける経路はフォロワー
