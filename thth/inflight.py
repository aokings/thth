...
# 「確かめてから解く」を 1 行で（設計 §3.5・3.3.1 §4・`docs/原稿_添付_2.13.md`・
...
# 解いてよいかを決められるのは画面を見た人だけ。ファイルを手で消す手順は
# 3.3.1 で `thth inflight <アカウント> resolve`（控えと変更ログが残る）に替えた。
NEXT_STEP = ("次の一歩: 前回の結果が分かっていません。thth inflight <アカウント> show で"
             "中身を見て、媒体の画面で出たかどうかを確かめ、thth inflight <アカウント> resolve"
             " --published <post_id> か --not-published で --by <名前> を付けて解いてから"
             "もう一度（確かめずに解かない）")
