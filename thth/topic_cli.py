...
def _recent_decisions(account: str) -> int:
    ...
    _emit({"ok": True, "account": account, "count": len(mine),
           ...
           #
           # **候補の数（`candidate_count`）は出さない——ただし理由は「重いから」
           # ではない**（訂正 2026-09-12）。
           #
           # この理由を 2 回書き間違えた。1 回目「数えるには `proposals` の棚を
           # 開く必要があるので重い」、2 回目（運用指摘を受けて）「単体取得は
           # 既にその経路を通っているので、正確には一覧で件数ぶん棚を開くのが
           # 重い」。**どちらも誤り。** 外部レビューの訂正で確かめた——
           # `cmd_decision` は保存済みの decision を読んで `freshness` を足す
           # だけで、**`proposals` の棚を開いていない。**
           #
           # **実際には `candidates` は decision の中に写しが入っている**
           # （`_envelope()` が返す dict をそのまま保存しているため）。つまり
           # **`len(r.get("candidates") or [])` で、棚を 1 つも開かずに数えられる。
           # 安い。**
           #
           # **それでも出さないのは、要る場面がまだ無いから**——外部レビューの
           # 裁定「利用者に必要な画面・処理が出た時点で検討してください」。
           # **「重いから見送る」と書くと、要る場面が来ても見送りが続く。**
