...
# 3.12.0 の台帳の項目 `approval`（none/publish/all）は 3.13.0 で無くなった（承認ページを消した）。
# 台帳に残っていても**読まずに通す**（`thth account status` が「古い項目」と 1 行言う）。
IGNORED_FIELDS = ("approval",)
...
def load_account(name: str) -> dict:
    ...
    **名前そのものを先に検査する**（セキュリティ監査 2026-09-14・P2-2）。
    ...
    # 安全装置（設計 3.12.0）。知らない値は黙って既定に倒さず断る。
    # `approval`（3.12.0）は読まない（IGNORED_FIELDS）。
